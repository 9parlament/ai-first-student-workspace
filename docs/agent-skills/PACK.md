...
`qa-make-full-pyramid` ≠ `qa-pyramid-plan` (план + одна дыра) и ≠ `qa-write-test` (один автотест): один вызов = один ярус уже влитой фичи, потом STOP. Контракт фичи — RAG (заметка: `note-http`), не таблица в generic skill.  
...
ADR курса (takeaway `docs/adr/`): [adr/005-screenshot-not-layer.md](adr/005-screenshot-not-layer.md), [adr/006-one-note-not-list.md](adr/006-one-note-not-list.md) (HTTP — RAG [`note-http`](rag/note-http.md)). Не путать с monorepo `docs/adr/005-testing-pyramid-review.md` / `006-allurerc-mjs-ethalon.md`. Проверяльщик школы (преподаватель): monorepo ADR 014 + skill `qa-homework-check`.
